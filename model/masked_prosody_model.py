...
class ConversationalMaskedProsodyModel(nn.Module):
    def __init__(
        self,
        args: ModelArgs,
    ):
        ...
        self.pitch_embedding = nn.Embedding(args.bins + 2, args.filter_size)
        self.energy_embedding = nn.Embedding(args.bins + 2, args.filter_size)
        self.vad_embedding = nn.Embedding(2 + 2, args.filter_size)
        self.positional_encoding = PositionalEncoding(args.filter_size)
        self.transformer = TransformerEncoder(
            ConformerLayer(
    ...
    def forward(self, x, condition=None, return_layer=None):
        ...
        vad = self.vad_embedding(vad)
        x = pitch + energy + vad
        condition_pitch = condition[:, 0]
        condition_energy = condition[:, 1]
        condition_vad = condition[:, 2]
        condition_pitch = self.pitch_embedding(condition_pitch)
        condition_energy = self.energy_embedding(condition_energy)
        condition_vad = self.vad_embedding(condition_vad)
        condition = condition_pitch + condition_energy + condition_vad
        # cross attention
        x = x.transpose(0, 1)
    ...
    def from_pretrained(path_or_hubid):
        ...
        margs.bins = args["bins"]
        margs.max_length = args["max_length"]
        model = ConversationalMaskedProsodyModel(margs)
        model.load_state_dict(torch.load(model_file))
        return model
